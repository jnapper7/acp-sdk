...
def test_models_generator_from_manifest(test_filename, model_ref_filename):
    curpwd = os.path.dirname(os.path.realpath(__file__))

    ref_models = os.path.join(curpwd, "test_samples", model_ref_filename)

    manifest = validate_agent_manifest_file(
        os.path.join(curpwd, "test_samples", test_filename)
    )
    tmp_dir = tempfile.TemporaryDirectory()
    generate_agent_models(manifest, tmp_dir.name, "models.py")
